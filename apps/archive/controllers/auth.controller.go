...
func (ac *AuthController) SignUpUser(ctx *gin.Context) {
	...
	// TODO  绑定git成功后才正式去改库,url得替换域名,下版本打开
	//url := "http://10.10.98.28:3000/api/v1/admin/users"
	//data := map[string]interface{}{}
	//data["email"] = newUser.Email
	//data["username"] = newUser.Name
	//data["password"] = payload.Password
	//jsonData, err := json.Marshal(data)
	//_, err = xhttp.Post(url, jsonData)
	//if err != nil {
	//	ctx.JSON(http.StatusConflict, gin.H{"status": "fail", "message": "User git with that email already exists, " +
	...
	emailData := utils.EmailData{
		URL:              configCopy.ClientOrigin + "/verifyemail/" + code,
		VerificationCode: code,
		FirstName:        firstName,
		Subject:          "Your account verification code",
	}

	go utils.SendEmail(&newUser, &emailData, "verificationCode.html")
...
func (ac *AuthController) SignInUser(ctx *gin.Context) {
	...
	ctx.SetCookie("access_token", accessToken, configCopy.AccessTokenMaxAge*60, "/", configCopy.Domain, false, true)
	ctx.SetCookie("refresh_token", refreshToken, configCopy.RefreshTokenMaxAge*60, "/", configCopy.Domain, false, true)
	ctx.SetCookie("logged_in", "true", configCopy.AccessTokenMaxAge*60, "/", configCopy.Domain, false, false)
...
func (ac *AuthController) LogoutUser(ctx *gin.Context) {
	...
	ctx.SetCookie("access_token", "", -1, "/", configCopy.Domain, false, true)
	ctx.SetCookie("refresh_token", "", -1, "/", configCopy.Domain, false, true)
	ctx.SetCookie("logged_in", "", -1, "/", configCopy.Domain, false, false)
...
func (ac *AuthController) ForgotPassword(ctx *gin.Context) {
	...
	go utils.SendEmail(&user, &emailData, "resetPassword.html")
...
func (ac *AuthController) ResetPassword(ctx *gin.Context) {
	...
	ctx.SetCookie("token", "", -1, "/", configCopy.Domain, false, true)
